...
* *Artsiom Skliar*
** *Major Enhancement*: incorporating balance, min, and max command, which calculates your overall balance as registered in TravelBanker.
...
financial situation. The min and max commands contribute to this goal by listing the largest amounts owed and lent.
...
** *Minor Enhancement*: modifying the select command to automatically search for a nearby atm when a contact is loaded.
This aims to help the user find ways to settle his finances quickly and efficiently.
...
* *Chen Chongsong*
** *Major Enhancement*: implementing a SortCommand to sort contact list according to balance, name, etc.
...
** *Minor Enhancement*: created a new command to delete specified tags (RemoveTagCommand).
This is important to the product, especially when the user wants to delete only one tag from a contact person who has many tags. Perviously, when using EditCommand to remove tags, the user needs to input all remaining tags. RemoveTagCommand, however, can be used by inputing the few tags that the user wants to delete. That improves efficiency to a great extent.
...
** *Minor Enhancement*: Create some helpful UI and UX improvements, such as color coded money figures depending on debt vs owing.
This is important to the product, because we would like to make sure that our users are able to quickly and efficiently interpret the information in our product. If our product was not pleasant to use, our users abandon the product altogether, or turn to other alternatives.
